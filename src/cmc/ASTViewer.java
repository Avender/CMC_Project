...
public class ASTViewer extends JFrame
{
    ...
    private DefaultMutableTreeNode createTree( AST ast )
    {
        ...
        if( ast == null )
            ...
            node.add( createTree( ((Block)ast).declarations ) );
            node.add( createTree( ((Block)ast).statements ) );
        } else if( ast instanceof Declarations ) {
            node.setUserObject( "Declarations" );

            for( Declaration d: ((Declarations)ast).declaration )
            ...
            node.add( createTree( ((ExpressionStatement)ast).expression) );
            ...
            node.add( createTree( ((BinaryExpression)ast).operand0 ) );
